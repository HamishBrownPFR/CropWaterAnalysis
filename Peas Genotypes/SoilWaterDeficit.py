...
# +
#Read in data
AllData=pd.read_csv('I:/Science Projects/I190910/Exception Files/04 Research/LoggedData/RainShelterAlpha_CS650.dat', #specify file path for data to read in
...
#Bring in index data
AllDataIndex=pd.read_csv('I:/Science Projects/I190910/Exception Files/04 Research/LoggedData/SoilWaterAndTempIndex.csv',
                         index_col = 0)
AllDataIndex.loc[:,'Irrigation'] = pd.Categorical(AllDataIndex.loc[:,'Irrigation'],['date','Low','Medium','Full'])
...
AllDataIndexed.set_index(['Measurement','Depth','Irrigation','Cultivar','Plot','Sensor', 'MUX', 'Port','Units','Summary','Block','Treatment'], 
...
DataMeans =  ObsData.groupby(level=['Depth','Cultivar','Irrigation'],axis=1).mean()
...
FieldCapacity = FieldCapacity.loc['2019-11-19']   # I would have though this would return a data frame with a single row but instead it returns a series with a multiindex in columns
